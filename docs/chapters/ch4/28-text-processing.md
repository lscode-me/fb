---
title: grep, sed, awk — руководство по обработке текста
description: Потоковая обработка текста в Linux. Поиск с grep, замена с sed, анализ с awk. Регулярные выражения, практические примеры.hide:
  - title---
