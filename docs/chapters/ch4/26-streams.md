...
### Ловушка: буферизация при записи в файл

Буферизация касается не только stdout — она влияет на **любой файловый I/O**. Вот пример, где буферизация ломает ожидания:
...
with p.open('a') as f1:
    print('a' * 200_000, file=f1)  # 200 001 байт (200000 + '\n')
    print('b' * 999, file=f1)      # 1000 байт
    # Дописываем в тот же файл из другого процесса
    subprocess.call("echo ccc >> qwe.txt", shell=True)

with p.open() as f1:
    # Ожидаем, что 'ccc' в самом конце — на позиции ~201001
    print(f1.read().index('ccc'))
```

Результат: `ccc` оказывается **между `a` и `b`** — на позиции 200000! Почему?

```
Что происходит по шагам:

1. print('a' * 200_000) → TextIOWrapper получает 200001 байт (200000 + '\n')
   → pending = 200000 байт ≥ chunk_size (8192) → flush в BufferedWriter
   → BufferedWriter: 200000 байт > buffer_size (128 КБ) → flush НА ДИСК
   → НА ДИСКЕ: 200000 байт (только 'a', без '\n'!)
   → TextIOWrapper: '\n' (1 байт) остаётся в pending

2. print('b' * 999) → TextIOWrapper получает 1000 байт
   → pending = 1 + 1000 = 1001 байт < 8192 → остаётся в pending
   → НА ДИСКЕ: по-прежнему 200000 байт

3. subprocess("echo ccc >> qwe.txt")
   → Отдельный процесс открывает файл с O_APPEND
   → Видит конец файла НА ДИСКЕ = позиция 200000
   → Пишет "ccc\n" → НА ДИСКЕ: 200004 байта

4. Выход из with → Python сбрасывает буферы:
   → TextIOWrapper flush: 1001 байт ('\n' + 'bbb...999\n') → BufferedWriter → диск
   → O_APPEND → дописывает после "ccc\n"
   → НА ДИСКЕ: 200004 + 1001 = 201005 байт
```
...
[aaa...200000 байт...][ccc\n][\nbbb...999...\n]
                       ^^^^
                  Позиция 200000 — не в конце!
```

Обратите внимание: `\n` от первого `print` **осталась в буфере** — `ccc` приклеилось прямо к `a`-блоку без разделителя! Это видно через `cut`:

```bash
$ cut -c 1-10 qwe.txt      # Первые 10 символов каждой строки
aaaaaaaaaa                  # Строка 1: 200003 символа (aaa...ccc) — ccc не видно!
                            # Строка 2: пустая (\n, дописанная позже)
bbbbbbbbbb                  # Строка 3: bbb...
```

Буферизация сломала не только **порядок**, но и **структуру строк** файла.

**Корень проблемы**: между Python и файловой системой — **два уровня невидимых буферов**:
...
print("data", file=f)
        ...
        ▼
┌─────────────────────────┐
│  TextIOWrapper          │ ← pending_bytes (до 8 КБ)
│  (кодирование + буфер)  │
├─────────────────────────┤
│  BufferedWriter         │ ← buffer (128 КБ в Python ≥ 3.14,
│  (основной буфер)       │    8 КБ в Python ≤ 3.13)
├─────────────────────────┤
│  FileIO  (raw)          │ ← без буфера, write() → syscall
├─────────────────────────┤
│  Ядро ОС (page cache)   │ ← ещё один уровень кэширования
├─────────────────────────┤
│  Диск                   │
└─────────────────────────┘
```

Другие процессы видят только то, что прошло через FileIO на диск. `'b'`-блок и `'\n'` от первого print целиком остались в буферах Python и **не покинули процесс** к моменту вызова `echo`.

!!! note "Размер буфера зависит от версии Python"
    В Python ≤ 3.13 `io.DEFAULT_BUFFER_SIZE` = 8192 (8 КБ). В Python 3.14 его **увеличили до 131072 (128 КБ)**. С меньшим первым блоком (например, `'a' * 9999`) результат отличается:
    
    - **Python 3.14** (128 КБ буфер): 10 000 байт помещаются в BufferedWriter → на диске 0 → `ccc` окажется на позиции 0 (в начале файла!)
    - **Python 3.13** (8 КБ буфер): 10 000 байт переполняют BufferedWriter → на диске ~8192 → `ccc` между `a`-блоками
    
    Один и тот же код — **разный порядок данных** в зависимости от версии.

**Решение** — явный `flush()` перед тем, как другой процесс обращается к файлу:

```python
with p.open('a') as f1:
    print('a' * 9999, file=f1)
    print('b' * 999, file=f1)
    f1.flush()  # Сбросить буфер Python → на диск
    subprocess.call("echo ccc >> qwe.txt", shell=True)
```

!!! warning "Общее правило"
    Если к файлу обращается **больше одного процесса** (или даже больше одного файлового объекта в одном процессе) — **не полагайтесь на буферизацию**. Явно вызывайте `flush()`, или открывайте файл с `buffering=0` (для бинарного режима) / `buffering=1` (построчная буферизация для текстового режима).
...
Оба устройства генерируют случайные данные из пула энтропии ядра, но ведут себя по-разному:

| Аспект | `/dev/random` | `/dev/urandom` |
|--------|--------------|----------------|
| **Блокировка** | Блокирует, если энтропии мало | Никогда не блокирует |
| **Источник** | Пул энтропии ядра | Тот же пул + CSPRNG |
| **Скорость** | Может быть медленным | Всегда быстрый |
| **Безопасность** | Криптографически стойкий | Криптографически стойкий* |

\* В **Linux ≥ 5.6** (2020) `/dev/random` больше не блокирует после инициализации пула — поведение фактически идентично `/dev/urandom`. Различие сохраняется только в старых ядрах и в **FreeBSD/macOS**, где `/dev/random` и `/dev/urandom` — синонимы (оба неблокирующие).

!!! tip "Что использовать?"
    **Почти всегда — `/dev/urandom`**. Он криптографически стойкий и не блокирует. Единственный случай, когда `/dev/random` может быть оправдан — генерация долгоживущих ключей на системе с очень ранней загрузкой (до накопления энтропии), и то в старых ядрах.

```bash
# Криптографически стойкие случайные байты
$ head -c 32 /dev/random | xxd

# То же самое, но гарантированно без блокировки
$ head -c 32 /dev/urandom | xxd

# Сгенерировать пароль
$ head -c 16 /dev/urandom | base64

# Python: правильный способ (использует getrandom() → urandom)
$ python3 -c "import secrets; print(secrets.token_hex(16))"
```

!!! warning "Распространённое заблуждение"
    Миф: «`/dev/urandom` небезопасен, нужен `/dev/random`». Это **неправда** — `urandom` использует тот же криптографический генератор. Разница лишь в поведении при пустом пуле энтропии (практически невозможно на работающей системе).
