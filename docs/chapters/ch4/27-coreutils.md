...
#### Практика: бинарный патч с dd

Представим задачу: у вас есть скомпилированный бинарь, в котором надо заменить несколько байтов, при этом окружение содержит только стандартные утилиты.

**Пример.** Есть программа, выводящая `hello dogs`. Изменим строку `dogs` на нули, чтобы вывелось просто `hello`.
...
**Шаг 1.** Найдём оффсет строки в бинарном файле:

```bash
$ gcc -o a.out hello.c

# Ищем позицию строки (с нулевым байтом в конце)
$ grep -P --byte-offset -o -a "dogs\x0" ./a.out
8202:dogs
```

**Шаг 2.** Применим патч с помощью `dd`:

```bash
# Заменяем 5 байт на нули, начиная с позиции 8202
$ dd if=/dev/zero of=./a.out bs=1 seek=8202 count=5 conv=notrunc
5+0 records in
5+0 records out
5 bytes copied, 0.000123 s, 40.7 kB/s
...
| Параметр | Значение |
|----------|----------|
| `if` | Файл с входными данными (`/dev/zero` — источник нулей) |
| `of` | Выходной файл (перезаписывается частично) |
| `bs=1` | Размер блока = 1 байт |
| `seek=8202` | Оффсет в выходном файле (откуда начинать запись) |
| `count=5` | Количество блоков (записать 5 байт) |
| `conv=notrunc` | **Критично!** Не обрезать файл после записи |

!!! warning "Флаг conv=notrunc обязателен"
    Без `conv=notrunc` dd обрежет выходной файл до `seek + bs × count` байт, и от бинаря останется только начало.

**Вариант с произвольными данными:**

Если нужно заменить не на нули, сначала запишем контент в файл:

```bash
$ printf "cats\0" > patch.bin

$ dd if=patch.bin of=./a.out bs=1 seek=8202 count=5 conv=notrunc
...
Для сравнения — то же самое на Python:
...
with open('a.out', 'rb') as f:
    data = f.read()
    offset = data.find(b'dogs\x00')
    print(f"Найдено на оффсете: {offset}")

# Патч: заменить на нули
with open('a.out', 'r+b') as f:
    f.seek(offset)
    f.write(b'\x00' * 5)

# Или заменить на другую строку
with open('a.out', 'r+b') as f:
    f.seek(offset)
    f.write(b'cats\x00')
```
...
| Критерий | bash + dd | Python |
|----------|-----------|--------|
| Зависимости | Только coreutils | Нужен Python |
| Читаемость | Низкая (магические параметры) | Высокая (явный код) |
| Скорость | Быстрее для больших файлов | Загружает весь файл в память |
| Портативность | POSIX-системы | Везде, где есть Python |
| Автоматизация | Сложнее (bash-скрипты) | Проще (полноценный язык) |

**Применение:**

- Патчинг бинарников в контейнерах без hex-редакторов
- Правка конфигурационных секций в firmware
- Восстановление повреждённых файлов
- Образовательные эксперименты с ELF/PE структурами
