...
## 45.7 Cloud-Native Storage: Signed URLs и Lifecycle

### Signed URLs — временный доступ без учётных данных
...
# Генерация URL для скачивания (действует 1 час)
url = s3.generate_presigned_url(
    "get_object",
...
print(url)
# https://my-bucket.s3.amazonaws.com/reports/q4.pdf?X-Amz-Signature=...

# Signed URL для загрузки (клиент может залить файл без AWS credentials)
...
```text
Стоимость хранения (примерные цены AWS, за ГБ/мес):

S3 Standard:       $0.023   ← Горячие данные
S3 Standard-IA:    $0.0125  ← Редкий доступ (мин. 30 дней)
S3 Glacier:        $0.004   ← Архив (извлечение: минуты-часы)
S3 Deep Archive:   $0.00099 ← Холодный архив (извлечение: 12 часов)
```

### S3-совместимые хранилища

| Решение | Self-hosted | Совместимость | Назначение |
|---------|-----------|---------------|-----------|
| **MinIO** | ✅ | Высокая | Production-ready, распределённое |
| **Ceph RadosGW** | ✅ | Хорошая | Часть Ceph (OpenStack) |
| **SeaweedFS** | ✅ | Базовая | Легковесный, быстрый |
| **GCS** (Google) | ☁️ | Через адаптер | Google Cloud Storage |
| **Azure Blob** | ☁️ | Через адаптер | Microsoft Azure |

```bash
# MinIO — локальный S3 за 1 минуту
docker run -p 9000:9000 -p 9001:9001 \
  minio/minio server /data --console-address ":9001"

# Используйте обычные S3-инструменты:
aws --endpoint-url http://localhost:9000 s3 ls
```
