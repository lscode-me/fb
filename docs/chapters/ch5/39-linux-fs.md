...
!!! info "Linux 7.0: Shared Page Cache"
    ...
    **Механизм работы:**
    ...
    - Каждому файлу назначается **fingerprint** (отпечаток) в extended attribute `trusted.erofs.fingerprint`
    - Fingerprint может быть криптографическим хэшем содержимого или просто уникальным числом
    - При открытии файла ядро создаёт внутренний deduplication inode, связанный с fingerprint
    - Последующие открытия файлов с тем же fingerprint используют тот же inode
    - Экономия памяти может достигать **50%** в контейнерных кластерах
    
    **Опции монтирования:**
    ```bash
    # Включить дедупликацию (требуется kernel 7.0+)
    mount -t erofs -o inode_share,domain_id=myapp rootfs.erofs /mnt
    
    # domain_id — изолирует пользователей (обязательна для безопасности)
    # Файлы с одним fingerprint дедуплицируются только внутри одного domain_id
    ```
    
    **Ограничения:**
    - ⚠️ Несовместимо с **direct I/O** (O_DIRECT)
    - ⚠️ Требуется `domain_id` для изоляции (без него злоумышленник может подменить содержимое)
    - Требуется kernel 7.0+ и xfsprogs 6.18+
