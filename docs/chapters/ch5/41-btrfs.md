...
### Проблема: тихое повреждение данных

ext4 и XFS решили проблему crash consistency через журналирование: при сбое питания метаданные остаются целыми. Но есть проблема похуже: диски могут **молча вернуть испорченные данные** (silent data corruption): сбой прошивки, bit flip от космического излучения, ошибка на шине. ext4 и XFS не хранят контрольных сумм данных, поэтому не могут обнаружить такое повреждение — приложение получит испорченный файл и не узнает об этом.

Btrfs решает эту проблему своей архитектурой: Copy-on-Write + checksums для каждого блока данных и метаданных. При чтении блока Btrfs сверяет checksum — если не совпадает, использует копию с другого диска (RAID) или сообщает об ошибке.
