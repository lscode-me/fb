...
Выбор ФС определяет ограничения на размер файлов, тома и длину имён:

| ФС | Макс. файл | Макс. том | Макс. имя | Примечание |
|----|-----------|----------|-----------|------------|
| **FAT32** | 4 ГБ | 2 ТБ | 255 байт | Самое частое ограничение: ISO/видео >4 ГБ |
| **exFAT** | 16 ЭБ | 128 ПБ | 255 символов | Замена FAT32 для флешек |
| **NTFS** | 16 ТБ¹ | 256 ТБ | 255 символов | ¹Windows ограничивает (теор. до 16 ЭБ) |
| **ext4** | 16 ТБ | 1 ЭБ | 255 байт | Стандарт Linux |
| **XFS** | 8 ЭБ | 8 ЭБ | 255 байт | Для больших файлов |
| **Btrfs** | 16 ЭБ | 16 ЭБ | 255 байт | CoW, снапшоты |
| **ZFS** | 16 ЭБ | 256×10¹⁵ ЭБ | 255 байт | Практически без ограничений |
| **APFS** | 8 ЭБ | 8 ЭБ | 255 символов | macOS/iOS |

!!! warning "FAT32 и файлы >4 ГБ"
    Нельзя записать ISO-образ или видео >4 ГБ на флешку с FAT32. Решение: отформатировать в exFAT (поддерживается всеми современными ОС).

!!! note "PATH_MAX"
    В Linux ограничение длины пути — `PATH_MAX = 4096` байт. Это ограничение ядра, а не файловой системы.
...
### Сколько дисковых операций стоит одно чтение?

Схема выше выглядит просто, но за каждым шагом скрываются **реальные дисковые I/O**. Чтобы открыть и прочитать файл `/home/user/document.txt`, ФС должна пройти весь путь по дереву каталогов:

```
open("/home/user/document.txt") + read()

Шаг   Что делает ФС                          Дисковый I/O
───── ──────────────────────────────────────── ────────────────
  1   Прочитать inode корневой директории /   read inode 2
  2   Прочитать данные /                      read data block
      (найти запись "home" → inode 100)
  3   Прочитать inode /home                   read inode 100
  4   Прочитать данные /home                  read data block
      (найти запись "user" → inode 250)
  5   Прочитать inode /home/user              read inode 250
  6   Прочитать данные /home/user             read data block
      (найти запись "document.txt" → inode 500)
  7   Прочитать inode document.txt            read inode 500
  8   Прочитать данные document.txt           read data block(s)
───── ──────────────────────────────────────── ────────────────
Итого: ~8 дисковых чтений для одного open() + read()
```

**8 дисковых операций** чтобы прочитать один файл! И это ещё без записи — если файл создаётся, нужно обновить inode, bitmap, данные директории, inode директории. Создание одного файла в один блок — это **минимум 5 записей** на диск.

Вот почему так важны:

- **Кэш inode и директорий** в памяти ядра — повторное обращение к `/home/user/` не требует дисковых операций
- **Группы блоков/цилиндров** (FFS, ext4) — размещают inode рядом с данными, сокращая seek
- **Буферизация записи** — ядро накапливает изменения в памяти и сбрасывает их на диск пачками

!!! info "Реальность vs теория"
    На практике большинство этих чтений обслуживаются из **page cache** ядра (кэш дисковых блоков в RAM). Inode корня `/` и популярных директорий `/home`, `/usr` почти всегда в кэше. Но при первом обращении или после перезагрузки — каждый шаг стоит реального seek'а.
