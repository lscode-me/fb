...
Традиционные ФС (ext4, XFS) решили проблему crash consistency с помощью журналирования. Но остаётся проблема, которую журнал не решает — **silent data corruption** (тихое повреждение данных).

Диски могут вернуть испорченные данные и **не сообщить об ошибке**. Причины: сбой прошивки диска (блок записан не туда), космическое излучение (bit flip), ошибка на шине при передаче, деградация магнитного слоя. Исследования (Bairavasundaram et al., CERN) показывают:

- **0.5%** SATA-дисков имеют хотя бы одно silent corruption за 3 года; для серверных дисков — ~0.05%
- **9.4%** SATA-дисков имеют latent sector errors (LSE) — блоки, которые диск не может прочитать

Problem: ext4 и XFS **не хранят контрольных сумм данных**. Если диск вернул испорченный блок — ФС не знает об этом и отдаёт мусор приложению. ZFS была создана именно для решения этой проблемы.
