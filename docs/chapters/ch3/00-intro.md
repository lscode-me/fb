...
### Навигатор: «Какой формат выбрать?»

Запутались в 10+ форматах? Вот decision matrix:

| Требование | Лучший выбор | Альтернатива |
|-----------|-------------|-------------|
| **Конфигурационный файл** | TOML | YAML (если нужны сложные структуры) |
| **API / обмен данными** | JSON | MessagePack (если важен размер) |
| **Конфигурация инфраструктуры** | YAML | JSON (для машинной генерации) |
| **Табличные данные для человека** | CSV/TSV | JSON Lines |
| **Табличные данные для аналитики** | Parquet | ORC (Hive-экосистема) |
| **RPC / межсервисное общение** | Protobuf + gRPC | Avro (Kafka), JSON (REST) |
| **Стриминг событий (Kafka)** | Avro + Schema Registry | Protobuf, JSON |
| **Хранение данных в одном файле** | SQLite | JSON (если данных мало) |
| **Архивирование / бэкап** | tar + zstd | zip (совместимость с Windows) |
| **Обмен документами со схемой** | XML + XSD | JSON + JSON Schema |
| **Медиа (фото, видео, аудио)** | Контейнер + кодек | Зависит от сценария (→ Глава 25) |

```text
                      Данные
                        │
              ┌─────────┴─────────┐
        Структурированные?    Медиа/бинарные?
              │                       │
        ┌─────┴─────┐           Контейнер+кодек
   Для человека? Для машины?    (→ Глава 25)
        │              │
   ┌────┴────┐    ┌────┴────┐
