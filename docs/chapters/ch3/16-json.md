...
!!! warning "Только двойные кавычки!"
    В JSON **все строки** (и значения, и ключи объектов) должны быть в **двойных кавычках** `"`. Одинарные кавычки `'` — синтаксическая ошибка.
    ...
    Это частая ошибка для тех, кто пришёл из Python (где `'строка'` допустима) или JavaScript (где ключи можно писать без кавычек: `{name: "Alice"}`).
