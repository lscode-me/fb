...
Все алгоритмы сжатия решают одну задачу: **записать ту же информацию меньшим количеством байтов**. Ключевая идея — найти и устранить **избыточность** в данных.

#### Идея 1: Частые символы → короткие коды (Huffman)

В обычном тексте одни символы встречаются чаще других. Но в ASCII каждый символ занимает ровно 8 бит — и частый пробел, и редкий `~`:

```
Текст: "ABRACADABRA" (11 символов × 8 бит = 88 бит)
...
**Идея Хаффмана (1952):** Дать частым символам короткие коды, редким — длинные.
...
Коды:              Стандартный ASCII:
  A → 0       (1 бит)     01000001 (8 бит)
  B → 100     (3 бита)    01000010 (8 бит)
  R → 101     (3 бита)    01010010 (8 бит)
  C → 110     (3 бита)    01000011 (8 бит)
  D → 111     (3 бита)    01000100 (8 бит)
```
...
= 23 бита (+ таблица кодов)

Было:  88 бит (ASCII)
Стало: 23 бита (+ ~40 бит на таблицу) ≈ 63 бита
Экономия: ~28%
```

!!! note "Префиксные коды"
    Коды Хаффмана — **префиксные**: ни один код не является началом другого. Поэтому поток бит `01001010110...` декодируется однозначно — читаем биты, пока не получим один из кодов, и начинаем заново.

#### Идея 2: Повторяющиеся фрагменты → ссылки (LZ77)

Тексты и данные полны повторов. Зачем записывать одно и то же дважды?

**LZ77 (Lempel-Ziv, 1977):** Заменяем повторяющийся фрагмент **ссылкой** на его предыдущее появление.

```
Исходный текст:
  "the cat sat on the mat on the flat"

Скользящее окно (sliding window):
  ← уже обработанные данные | текущая позиция →

Сжатие:
  "the cat sat on "        ← записываем как есть
  (15, 4)                  ← "the " — было 15 символов назад, длина 4
  "mat on "                ← как есть
  (8, 4)                   ← "the " — было 8 символов назад
  "flat"                   ← как есть
```

В итоге вместо текста хранится смесь литералов и ссылок `(расстояние, длина)`:

```
Было:  "the cat sat on the mat on the flat"  (35 символов)
Стало: "the cat sat on " + (15,4) + "mat on " + (8,4) + "flat"

Чем больше повторов — тем лучше сжатие.
```

!!! example "Почему TAR.GZ сжимает лучше ZIP"
    Когда 1000 похожих лог-файлов объединены в один поток (TAR), LZ77 видит повторы **между файлами** — одинаковые заголовки, форматы строк, IP-адреса. В ZIP каждый файл сжимается отдельно, и повторы между файлами теряются.

#### Идея 3: Комбинируем оба подхода (DEFLATE)

Большинство реальных алгоритмов **комбинируют** методы:

```
DEFLATE (gzip, ZIP, PNG):

Данные → LZ77 (убирает повторы) → Huffman (оптимизирует коды) → Сжатый поток
         ^^^^^^^^^^^^^^^^^^^^     ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
         Словарное сжатие          Энтропийное кодирование
```

| Этап | Что делает | Пример |
|------|-----------|--------|
| **LZ77** | Заменяет повторы ссылками | `"abcabc"` → `"abc" + (3,3)` |
| **Huffman** | Частым символам — короткие коды | `A` → `0`, `Z` → `11011010` |

#### Почему одни данные сжимаются хорошо, а другие нет?

Всё определяется **избыточностью** (redundancy):

```
Хорошо сжимается:              Плохо сжимается:
─────────────────              ──────────────────
• Текст (много повторов)       • Случайные данные (нет паттернов)
• Логи (одинаковый формат)     • Уже сжатые файлы (gzip, mp3)
• Исходный код (шаблоны)       • Шифрованные данные (похожи на шум)
• Нули (файлы с дырами)        • Архивы внутри архивов

"aaaaaaaaaa" → сжимается в ~1/10
"k8$mQ2!xLp" → почти не сжимается
```

```python
import gzip

# Повторяющиеся данные — отлично сжимаются
repetitive = b"Hello world! " * 10000  # 130 000 байт
compressed = gzip.compress(repetitive)
print(f"Повторы: {len(repetitive)} → {len(compressed)} байт "
      f"({len(compressed)/len(repetitive)*100:.1f}%)")
# Повторы: 130000 → 175 байт (0.1%)

# Случайные данные — не сжимаются
import os
random_data = os.urandom(130000)  # 130 000 случайных байт
compressed = gzip.compress(random_data)
print(f"Случайные: {len(random_data)} → {len(compressed)} байт "
      f"({len(compressed)/len(random_data)*100:.1f}%)")
# Случайные: 130000 → 130077 байт (100.1%) — даже чуть больше!
```

!!! warning "Не сжимайте дважды"
    Повторное сжатие бесполезно — уже сжатые данные выглядят как «шум» и не уменьшаются. Именно поэтому `.tar.gz.gz` не имеет смысла, а `.jpg` файлы в ZIP хранятся без сжатия (store).

#### Сравнение алгоритмов: что внутри

| Алгоритм | Метод | Используется в |
|----------|-------|---------------|
| **DEFLATE** | LZ77 + Huffman | gzip, ZIP, PNG, HTTP |
| **BWT + Huffman** | Блочная сортировка + Huffman | bzip2 |
| **LZMA/LZMA2** | LZ77 + range coding (улучшенный Huffman) | xz, 7z |
| **Zstandard** | LZ77 + ANS (Asymmetric Numeral Systems) | zstd, Facebook |
| **LZ4** | Упрощённый LZ77 | Ядро Linux, базы данных |
| **Brotli** | LZ77 + Huffman + предзаданный словарь | HTTP (веб-серверы) |
