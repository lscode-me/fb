...
Способ хранения записей директории (directory entries) — одно из ключевых архитектурных решений файловой системы. От него напрямую зависит скорость операций: поиск файла по имени, создание, удаление и листинг содержимого. Чем больше файлов в директории, тем сильнее проявляется разница:
...
В линейном списке (ext2/ext3) поиск файла требует перебора всех записей — $O(n)$. При 10 000 файлах в директории это становится заметно. Древовидные структуры (B-дерево, B+ дерево, HTree) обеспечивают поиск за $O(\log n)$, что критично для больших директорий.

!!! info "Подробнее о файловых системах"
    Архитектура каждой ФС, их внутреннее устройство и сравнение — в [Главе 5. Файловые системы](../ch5/00-intro.md).

**HTree в ext4** — это хеш-дерево (hash tree), специальная индексная структура для ускорения поиска файлов в директориях. Вместо линейного перебора всех записей, ext4 вычисляет хеш от имени файла и по нему находит нужный блок директории за одно-два обращения к диску. HTree включается через feature-флаг `dir_index` и в современных системах активен по умолчанию:

```bash
# ext4: проверить, что HTree (dir_index) включён
$ tune2fs -l /dev/sda1 | grep dir_index
Filesystem features: ... dir_index ...

# Включить dir_index, если отключён (на отмонтированной ФС)
$ tune2fs -O dir_index /dev/sda1
$ e2fsck -D /dev/sda1   # пересоздать индексы директорий
```
...
!!! info "Подробнее о ссылках"
    Здесь мы рассмотрели только **содержимое** symlink — путь внутри файла. Полное сравнение hard links и symbolic links, битые ссылки (dangling), циклические ссылки, относительные vs абсолютные пути, права доступа и подводные камни — в [Главе 4. Ссылки](04-links.md).
...
!!! info "Подробнее"
    Как устройства автоматически появляются в `/dev` (udev, devtmpfs) — в [Главе 1. Определение файла](01-file-definition.md). Роль `/dev/null`, `/dev/zero`, `/dev/random` и `/dev/urandom` как потоков данных — в [Главе 26. Потоки](../ch4/26-streams.md).
...
!!! info "Подробнее"
    Детали работы pipe (буфер, блокировка, deadlock) — в [Главе 26. Потоки](../ch4/26-streams.md). Анонимные и именованные pipe, Unix domain sockets (stream vs datagram, сравнение с TCP, Python-примеры) — в [Главе 7. Файловые дескрипторы](07-descriptors.md).

---

## 2.6 Исполняемые файлы
...
!!! tip "Подробнее"
    Как ядро решает, чем запустить файл (magic bytes, shebang `#!`) — в [Главе 6. Права доступа](06-permissions.md). Детальная структура ELF, Mach-O и PE (секции, заголовки, magic numbers) — в [Главе 22. Бинарная структура файлов](../ch3/22-binary-structure.md).

---

## 2.7 Файловые операции по типам
