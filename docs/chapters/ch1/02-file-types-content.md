...
## 2.7 Исполняемые файлы: ELF и Mach-O
...
**Executable and Linkable Format** — стандартный формат исполняемых файлов в Linux, FreeBSD, Solaris.

```bash
# Magic number: 7f 45 4c 46 = \x7fELF
$ xxd /bin/ls | head -1
...
# Заголовки и секции
$ readelf -h /bin/ls    # Заголовок ELF
$ readelf -S /bin/ls    # Секции (.text, .data, .rodata, .bss)
$ readelf -l /bin/ls    # Сегменты (program headers)
```
...
```
┌──────────────────┐
│   ELF Header     │  Magic, архитектура, entry point
├──────────────────┤
│ Program Headers  │  Описание сегментов для загрузки
├──────────────────┤
│ .text            │  Машинный код
│ .rodata          │  Константы (строки, таблицы)
│ .data            │  Инициализированные глоб. переменные
│ .bss             │  Неинициализированные переменные (нули)
│ .symtab          │  Таблица символов
│ .strtab          │  Строки (имена символов)
├──────────────────┤
│ Section Headers  │  Описание секций для линковки
└──────────────────┘
```

### Mach-O (macOS/iOS)

**Mach Object** — формат исполняемых файлов Apple.

```bash
# Magic: CF FA ED FE (little-endian) или FE ED FA CF (big-endian)
$ xxd /bin/ls | head -1
...
# Заголовки
$ otool -h /bin/ls     # Mach header
$ otool -l /bin/ls     # Load commands
```

!!! info "Universal Binary (Fat Binary)"
    macOS поддерживает **Universal Binary** — один файл содержит код для нескольких архитектур (x86_64 + arm64). При запуске ОС выбирает нужную. Размер файла удваивается, но обеспечивает совместимость.

### Сравнение

| | ELF | Mach-O | PE (Windows) |
|---|---|---|---|
| ОС | Linux, BSD | macOS, iOS | Windows |
| Magic | `\x7fELF` | `\xcf\xfa\xed\xfe` | `MZ` |
| Утилиты | `readelf`, `objdump` | `otool`, `nm` | `dumpbin` |
| Расширение | нет (или `.so`) | нет (или `.dylib`) | `.exe`, `.dll` |
| Мульти-архитектура | Нет | Universal Binary | Нет |
...
## 2.9 Таблица лимитов файловых систем

Разные ФС имеют разные ограничения. Выбор ФС может зависеть от размера файлов и тома:

| ФС | Макс. размер файла | Макс. размер тома | Макс. длина имени | Макс. глубина пути |
|----|-------------------|-------------------|-------------------|--------------------|
| **FAT32** | 4 ГБ | 2 ТБ | 255 байт | Не определена |
| **exFAT** | 16 ЭБ | 128 ПБ | 255 символов | Не определена |
| **NTFS** | 16 ТБ¹ | 256 ТБ | 255 символов | 32 767 символов |
| **ext4** | 16 ТБ | 1 ЭБ | 255 байт | Не ограничена² |
| **XFS** | 8 ЭБ | 8 ЭБ | 255 байт | Не ограничена² |
| **Btrfs** | 16 ЭБ | 16 ЭБ | 255 байт | Не ограничена² |
| **ZFS** | 16 ЭБ | 256 × 10¹⁵ ЭБ | 255 байт | Не ограничена² |
| **APFS** | 8 ЭБ | 8 ЭБ | 255 символов (UTF-8) | Не ограничена² |

¹ NTFS теоретически поддерживает до 16 ЭБ, но Windows ограничивает до 256 ТБ.

² В Linux ограничение PATH_MAX = 4096 байт, но это ограничение ядра, а не ФС.

!!! warning "FAT32 и файлы >4 ГБ"
    Это самая частая проблема: нельзя записать ISO-образ или видео >4 ГБ на флешку с FAT32. Решение: отформатировать в exFAT (поддерживается всеми современными ОС).
