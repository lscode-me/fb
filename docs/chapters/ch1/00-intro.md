...
??? example "Попробуйте сами: задания по Части I"
    Эти задания объединяют материал нескольких глав. Выполняйте после прочтения всей части.

    **1. Аудит файловой системы (исследование)**
    Выберите директорию проекта и составьте полный «паспорт» 3 файлов: тип (`file`), inode (`stat`), права, владелец, размер, жёсткие/символические ссылки, расширенные атрибуты. Оформите как таблицу.

    **2. Детектив дескрипторов (практика)**
    Запустите любой долгоживущий процесс (например, `python3 -c "import time; open('/tmp/test','w'); time.sleep(300)"`) и исследуйте его дескрипторы через `/proc/PID/fd` (Linux) или `lsof -p PID`. Сколько fd открыто? Что каждый из них представляет?

    **3. Ловушка с правами (кодинг)**
    Напишите скрипт, который создаёт файл, ставит на него `chmod 000`, пытается прочитать (должна быть ошибка), восстанавливает права и читает снова. Повторите от `root` — в чём разница?

    **4. Путь байта (сквозное задание)**
    Проследите путь данных от `echo "hello" > /tmp/test.txt` до диска: какие системные вызовы выполняются (`strace`), какой inode получит файл, какие блоки будут выделены, какой дескриптор использован? Используйте `strace`, `stat`, `debugfs` (для ext4).
