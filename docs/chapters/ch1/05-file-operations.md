...
### Откуда берутся названия команд?

Unix-команды славятся своей краткостью — часто это сокращения до 2–4 букв. Это наследие 1970-х, когда терминалы были медленными (10 символов в секунду на телетайпе), а каждый нажатый символ стоил времени. Кен Томпсон и Деннис Ритчи намеренно делали команды короткими.

| Команда | Расшифровка | Происхождение |
|---------|-------------|---------------|
| `touch` | *touch* | прикоснуться |
...
| `ln` | **l**i**n**k | Создать ссылку |
| `ls` | **l**i**s**t | Вывести список файлов |
| `cat` | con**cat**enate | Объединить (и вывести) содержимое файлов |
| `chmod` | **ch**ange **mod**e | Изменить режим доступа |
| `chown` | **ch**ange **own**er | Изменить владельца |

Обратите внимание на паттерн: гласные выбрасываются первыми (*remove → rm*, *move → mv*, *copy → cp*). Это не случайность — в коротких словах согласные несут больше информации, чем гласные, и слово остаётся узнаваемым.

!!! note "Почему `unlink`, а не `ul`?"
    Некоторые команды сохранили полное имя. `unlink` — это **прямое отражение системного вызова** `unlink()`. Команды, напрямую соответствующие syscall, часто носят их полное имя: `unlink`, `mount`, `stat`, `sync`.
...
Название `touch` — одно из самых поэтичных в Unix. Представьте стопку бумаг на столе: если вы «прикоснулись» к документу, на нём остаётся свежая отметка времени — вы его «потрогали», подтвердив, что он актуален. Именно это и делает команда: обновляет временные метки файла (atime, mtime), не меняя содержимого.

Создание нового файла — это **побочный эффект**, а не основная задача. Если указанный файл не существует, `touch` создаёт его (пустым), чтобы было куда записать timestamp. На практике именно этот побочный эффект используется чаще всего.
...
`mkdir` расшифровывается как **make directory** — «создать директорию». Команда делает ровно то, что говорит её имя: создаёт новую пустую директорию. Флаг `-p` (*parents*) — один из самых полезных: он создаёт всю цепочку родительских директорий и не выдаёт ошибку, если директория уже существует.
...
`rmdir` — **remove directory**. Намеренно работает только с пустыми директориями — это защита от случайного удаления. Логика проста: если в директории что-то лежит, значит, вы должны **явно** решить, что делать с содержимым.
...
`cp` — сокращение от **copy**. Это одна из самых «дорогих» файловых операций: в отличие от `mv`, `cp` должна физически прочитать все данные из источника и записать их в новое место. Создаётся **полностью независимый файл** с новым inode и новыми блоками данных.
...
`mv` — сокращение от **move** (переместить). Но название немного обманчиво: в пределах одной файловой системы `mv` **не перемещает данные** вообще. Он только переписывает запись в директории — меняет имя или «перекладывает» запись из одной директории в другую. Поэтому `mv` гигабайтного файла внутри одной ФС выполняется мгновенно.

`mv` также выполняет роль команды **rename** — в Unix нет отдельной утилиты для переименования (хотя существует пользовательская `rename` из пакета `util-linux`).
...
`rm` — **remove** (удалить). Но Unix-смысл «удаления» отличается от бытового: `rm` не стирает данные с диска, а **удаляет запись (имя) из директории** и уменьшает счётчик ссылок в inode. Данные исчезнут, только когда на них не останется ни одной ссылки и ни одного открытого дескриптора.

`unlink` — ещё более честное название: оно буквально значит «отсоединить» (*un-link*) — разорвать связь между именем файла и его inode. Это **прямой аналог системного вызова** `unlink()`.
...
`ln` — сокращение от **link** (ссылка). Команда создаёт дополнительное имя для данных — это как присвоить человеку псевдоним: он остаётся тем же человеком, просто откликается на другое имя.

По умолчанию `ln` создаёт **hard link** — ещё одну запись в директории, указывающую на тот же inode. С флагом `-s` (*symbolic*) создаётся **символическая ссылка** — отдельный файл, содержащий путь к цели.

Фундаментальная разница: hard link — это **равноправное имя** (нет «оригинала» и «копии»), а symlink — это **указатель**, который может «висеть в воздухе», если цель удалена.
...
Основные файловые операции (`cp`, `mv`, `rm`, `mkdir`) существуют на всех ОС, но реализации различаются. На Linux используются **GNU coreutils**, на macOS и FreeBSD — **BSD-версии** тех же утилит. Опции часто пересекаются, но не полностью: например, `cp --preserve=all` работает в GNU, а в BSD аналог — `-p`. PowerShell в Windows использует совершенно другой синтаксис с полными словами (`Copy-Item`, `Move-Item`).
...
Аналогия: представьте кабинет врача. **Advisory lock** — это табличка «Приём» на двери, которую коллеги уважают, но физически дверь не заперта. **Mandatory lock** — это настоящий замок на двери. Unix исторически предпочитает advisory-подход: ответственность за координацию лежит на программистах, а не на ОС.

Название `flock` — сокращение от **file lock**. Команда `fcntl` — от **file control** (управление файлом).
...
Как программы узнают, что файл изменился, **не опрашивая его в цикле**? Простейший способ — проверять файл каждые N секунд (`polling`), но это расточительно: тысячи ненужных системных вызовов, нагрузка на CPU, задержка обнаружения. Поэтому операционные системы предоставляют механизмы push-уведомлений — ядро само сообщает, когда что-то изменилось:

Названия отражают суть: **inotify** = *inode notify* (уведомление об inode), **fanotify** = *filesystem-wide access notify*, **kqueue** = *kernel event queue* (очередь событий ядра).
