...
```mermaid
graph TD
    A["Приложение<br/><code>read(fd, buf, n)</code>"] --> B["Файловый дескриптор<br/>(fd table процесса)"]
    B --> C["Open File Table<br/>(системная, общая)"]
    ...
    G --> I
    H --> J["Сеть"]
    I --> K["Драйвер диска<br/>(SCSI / NVMe)"]
    K --> L["💾 Физический диск"]
...
## 7.8 Memory-Mapped Files (mmap)

Обычно работа с файлом выглядит так: **`read()` → буфер в user-space → обработка → `write()`**. При использовании **mmap** файл (или его часть) проецируется прямо в адресное пространство процесса — данные доступны как обычный массив байтов, без явных системных вызовов чтения/записи.

```text
┌─────────────────────────────────────┐
│         Адресное пространство       │
│  ┌───────────┐   ┌──────────────┐   │
│  │   Код     │   │  mmap region │◄──┼── Файл на диске
│  └───────────┘   └──────────────┘   │
│  ┌───────────┐   ┌──────────────┐   │
│  │   Heap    │   │    Stack     │   │
│  └───────────┘   └──────────────┘   │
...
### Когда mmap эффективнее read/write

| Сценарий | `read()`/`write()` | `mmap` |
|----------|-------------------|--------|
| Чтение всего файла последовательно | ✅ Отлично | ≈ Равнозначно |
| Случайный доступ к большому файлу | ❌ Много `lseek` + `read` | ✅ Просто обращение по индексу |
| Разделяемый доступ между процессами | ❌ Нужны pipe/socket | ✅ `MAP_SHARED` |
| Файл очень маленький (< 4 КБ) | ✅ Проще и быстрее | ❌ Overhead на создание маппинга |
| Изменение размера файла | ✅ Просто `write()` | ❌ Нужен `mremap` / пересоздание |
...
with open("data.bin", "r+b") as f:
    ...
    # Доступ как к массиву байтов
    print(mm[:10])           # первые 10 байт
    print(mm[1000:1010])     # произвольный срез — без lseek!
    
    # Поиск
    pos = mm.find(b"HEADER")
    if pos != -1:
        print(f"Найдено на позиции {pos}")
    
    # Запись (файл должен быть открыт в r+b)
    mm[0:5] = b"HELLO"
...
# Создаём анонимный mmap (без файла) для IPC
shared = mmap.mmap(-1, 1024, tagname="my_shared")  # Windows
# На Unix:
# shared = mmap.mmap(-1, 1024)

shared.write(b"Message from parent")
shared.seek(0)

pid = os.fork()
if pid == 0:
    # Дочерний процесс читает те же данные
    print(shared.readline())
```

!!! warning "Подводные камни mmap"
    
    **Truncation (усечение файла)**: Если другой процесс уменьшит файл, а вы обратитесь к «исчезнувшей» области — получите **SIGBUS** (аварийное завершение). Решение: используйте файловые блокировки (→ Глава 5) или проверяйте размер перед доступом.
    
    **Coherence (когерентность)**: Изменения через `mmap` видны другим процессам с `MAP_SHARED`, но **не гарантируется** немедленная запись на диск. Вызывайте `mm.flush()` для гарантии.
    
    **32-bit ограничение**: На 32-битных системах адресное пространство — 4 ГБ. Маппинг файла > ~2 ГБ невозможен. На 64-bit таких проблем нет.
