...
Но прежде чем говорить о современных файлах, стоит понять, **откуда взялась сама идея файла** — потому что это не очевидная и не единственная модель работы с данными.

## 1.1 Краткая история: от перфокарт к файлам

### Мир без файлов (1940-е — 1950-е)

Первые компьютеры не имели файлов, файловых систем и даже операционных систем. Данные хранились на **физических носителях**, каждый из которых обрабатывался как единое целое:

```
1940-е  Перфокарты (punched cards)
        ┌──────────────────────────────────────┐
        │ ○  ○○ ○  ○○○ ○○  ○  ○○ ○  ○○  ○○ ○   │  80 колонок
        │○ ○  ○ ○○  ○  ○ ○○ ○○  ○ ○○  ○ ○  ○○  │  12 строк
        └──────────────────────────────────────┘
        Одна карточка = одна строка кода или данных
        Программа = стопка карточек (deck)
        Уронил стопку — собирай и сортируй заново

1950-е  Магнитные ленты (magnetic tapes)
        ┌──┬──────────┬──┬──────────┬──┬──────────┬──┐
        │▒▒│ Record 1 │▒▒│ Record 2 │▒▒│ Record 3 │▒▒│
        └──┴──────────┴──┴──────────┴──┴──────────┴──┘
         gap            gap            gap
        Последовательный доступ: чтобы попасть к Record 3,
        нужно промотать через Record 1 и Record 2
```

**Перфокарты** (IBM, с 1928 года, формат восходит к Жаккардовому станку 1804 года) — картонные карточки с пробитыми отверстиями. Стандарт IBM: 80 колонок × 12 строк. Программист писал программу на бланках, оператор пробивал карты на перфораторе, стопку передавали оператору ЭВМ. Через часы (или дни) получали распечатку результата. Одна колода карт — одна программа или набор данных.

**Магнитные ленты** (UNIVAC I, 1951) — первый массовый магнитный носитель. Данные записывались **последовательно** блоками (records), разделёнными промежутками (inter-record gaps). Доступ строго последовательный — чтобы добраться до нужной записи, лента перематывалась с начала. Именно записи на магнитных лентах начали называть **файлами** — по аналогии с папками-скоросшивателями в картотеке (англ. *file* — подшивка).

!!! note "Наследие перфокарт"
    80 колонок перфокарты — причина, по которой терминалы имели ширину 80 символов, а многие стандарты кодирования (FORTRAN, COBOL) ограничивали строку 80 символами. Этот призрак живёт до сих пор: PEP 8 рекомендует 79 символов, а GitHub показывает diff в колонке ~80 символов.

### Магнитные диски и рождение файловых систем (1956 — 1965)

Революция произошла с появлением **магнитных дисков** — впервые стал возможен **произвольный доступ** к данным:

```
1956  IBM 305 RAMAC — первый жёсткий диск
      50 алюминиевых пластин, 24 дюйма каждая
      Ёмкость: 5 МБ (примерно одна MP3-песня)
      Вес: около 1 тонны
      Стоимость аренды: $3 200/мес (≈ $35 000 в сегодняшних деньгах)

1961  CTSS (Compatible Time-Sharing System, MIT)
      Первая система с:
        ✓ Именованными файлами
        ✓ Каталогами (один уровень — у каждого пользователя свой)
        ✓ Разделением доступа между пользователями
        ✓ Командной строкой для работы с файлами

1965  Multics (Bell Labs + MIT + GE)
      Первая система с:
        ✓ Иерархической файловой системой (вложенные каталоги)
        ✓ Правами доступа (ACL)
        ✓ Символическими именами и путями
        ✓ Динамическим связыванием файлов
```

**CTSS** (1961) — поворотный момент. До CTSS компьютер обрабатывал задания пакетами (batch processing): оператор загружал колоду перфокарт, машина работала, выдавала результат. CTSS позволила **нескольким пользователям одновременно** работать с машиной через терминалы. Каждому нужно было место для хранения своих программ и данных — так появились **файлы с именами** и **личные каталоги**.

**Multics** (1965) пошёл дальше: вложенные каталоги, права доступа, единое адресное пространство для файлов и памяти. Проект оказался чрезмерно сложным, но его идеи легли в основу того, чем мы пользуемся сегодня.

### Unix: файл как универсальная абстракция (1969 — 1970-е)

Когда Кен Томпсон и Деннис Ритчи, разочаровавшись в Multics, начали создавать Unix (1969), они взяли ключевые идеи Multics, но **радикально их упростили**:

| Multics | Unix | Что изменилось |
|---------|------|----------------|
| Сложная система сегментов и страниц | Плоский поток байтов | Файл — просто последовательность байтов, без структуры |
| Развитые ACL | rwx для owner/group/others | Простая, но достаточная модель прав |
| Динамическое связывание | Жёсткие ссылки (hard links) | Минимализм |
| Многоуровневая безопасность | Один UID/GID | Достаточно для большинства задач |

Главное нововведение Unix — идея **«всё является файлом»** (everything is a file). Вместо отдельных API для дисков, терминалов, принтеров и сети Unix предложил **единый интерфейс** из четырёх операций:
...
Этот минимализм оказался гениальным. Unix распространился из Bell Labs в университеты, затем — в коммерческие системы, а его потомки (Linux, macOS, FreeBSD, Android, iOS) сегодня работают на подавляющем большинстве устройств в мире.

### Хронология ключевых событий

```
1804  Жаккардовый станок — перфокарты управляют ткацким станком
1890  Табулятор Холлерита — перфокарты для переписи населения США
1928  IBM стандартизирует 80-колоночную перфокарту
1951  UNIVAC I — первый компьютер с магнитной лентой
1956  IBM RAMAC — первый жёсткий диск (5 МБ)
1961  CTSS — первая ОС с именованными файлами и каталогами
1965  Multics — иерархическая ФС, права доступа (ACL)
1969  Unix — «всё есть файл», плоский поток байтов
1973  Unix переписан на C — переносимость ФС
1978  BSD Unix — быстрая файловая система (FFS)
1983  Sun NFS — сетевой доступ к файлам
1984  Macintosh — графический файловый менеджер (Finder)
1991  Linux — ext файловая система
1993  Windows NT — NTFS (журналирование, ACL, потоки данных)
2001  macOS — HFS+ (затем APFS в 2017)
2005  ZFS (Sun) — checksums, snapshots, pooled storage
2006  Amazon S3 — файлы → объекты в облаке
2007  Btrfs (Oracle/Linux) — CoW, snapshots, compression
2014  OverlayFS (Linux 3.18) — слоёная ФС, основа Docker-контейнеров
```

---

## 1.2 «Everything is a file» — философия Unix
...
## 1.3 Формальное определение файла
...
## 1.5 Типы файлов в Unix/Linux/BSD на практике
...
## 1.7 Псевдофайлы: «everything is a file» в действии
...
## 1.9 Создание разных типов файлов
