...
### NUL — нулевой байт (0x00)

NUL — самый первый символ ASCII и один из самых важных. Его описание «конец строки в C» — лишь верхушка айсберга. Нулевой байт играет фундаментальную роль как **терминатор** и **разделитель** во множестве контекстов.

#### C-строки: нулевой терминатор

В языке C строка — это просто последовательность байтов, заканчивающаяся `\0`. Функции `strlen()`, `strcmp()`, `printf("%s")` — все ищут нулевой байт как маркер конца:

```c
char greeting[] = "Hello";
// В памяти: 48 65 6C 6C 6F 00
//           H  e  l  l  o  \0
// sizeof(greeting) = 6 (включая \0)
// strlen(greeting) = 5 (без \0)
```

Это решение Кена Томпсона и Денниса Ритчи (1972) имело далеко идущие последствия: NUL стал единственным байтом, который **невозможен внутри** C-строки. Отсюда — и запрет NUL в именах файлов Unix, и дизайн UTF-8, и [Modified UTF-8 в Java](14-unicode-utf8.md#modified-utf-8-mutf-8).

#### Два запрещённых символа в именах файлов Unix

В Unix имя файла может содержать **любые** байты, кроме двух (подробнее — [глава 8: ограничения имён](../ch1/08-paths-names.md#83-ограничения-имён-файлов)):

| Символ | Причина запрета |
|--------|-----------------|
| `/` (0x2F) | Разделитель путей |
| NUL (0x00) | Терминатор C-строк в системных вызовах `open()`, `stat()`, `unlink()` и т.д. |

Ядро передаёт имена файлов как `char*` — NUL невозможен физически.

#### `/proc` — NUL как разделитель полей

Linux-ядро использует NUL вместо `\n` в нескольких файлах `/proc` (см. [псевдофайлы /proc](../ch1/02-file-types-content.md#23-псевдофайлы-proc-и-sys)), потому что значения полей сами могут содержать пробелы и переносы строк:

```bash
# /proc/PID/environ — переменные окружения, разделённые \0
...
# /proc/PID/cmdline — аргументы командной строки, разделённые \0
$ cat /proc/1234/cmdline | tr '\0' ' '
/usr/bin/python3 script.py --verbose

# Почему не \n? Потому что переменная окружения МОЖЕТ содержать \n:
$ export MULTILINE=$'line1\nline2'
$ cat /proc/self/environ | tr '\0' '\n' | grep MULTILINE
MULTILINE=line1  # ← Оборвалась! \n внутри значения стал разделителем
# NUL — единственный безопасный разделитель
```

Файлы `/proc` с NUL-разделителями:

| Файл | Содержимое | Разделитель |
|------|------------|-------------|
| `/proc/PID/environ` | Переменные окружения `KEY=VALUE` | `\0` |
| `/proc/PID/cmdline` | Аргументы командной строки | `\0` |
| `/proc/PID/numa_maps` | Информация NUMA (некоторые поля) | `\0` |

#### `find -print0` / `xargs -0` / `sort -z` — NUL-safe пайплайны

Поскольку имена файлов могут содержать `\n`, пробелы, кавычки и любые другие символы (кроме `/` и `\0`), NUL — единственный **гарантированно безопасный** разделитель (см. также [нулевой разделитель в потоках](../ch4/26-streams.md#нулевой-разделитель-0-для-безопасности) и [xargs -0](../ch4/27-coreutils.md#xargs--p--параллельный-xargs)):

```bash
# ОПАСНО: файл "my file\nbackup.txt" сломает обработку
$ find . -name "*.txt" | xargs rm

# БЕЗОПАСНО: NUL-разделитель
$ find . -name "*.txt" -print0 | xargs -0 rm

# GNU coreutils поддерживают NUL повсеместно:
$ find . -print0 | sort -z            # sort -z (--zero-terminated)
$ find . -print0 | grep -zl "pattern" # grep -z / -Z
$ find . -print0 | head -z -n 5       # head -z (--zero-terminated)
$ find . -print0 | shuf -z            # shuf -z
$ find . -print0 | uniq -z            # uniq -z

# Чтение NUL-разделённых записей в bash
$ while IFS= read -r -d '' file; do
    echo "Processing: $file"
done < <(find . -name "*.log" -print0)
```

#### NUL в бинарных форматах

Нулевой байт широко используется как терминатор и паддинг в бинарных форматах (подробнее — [глава 22](../ch3/22-binary-structure.md)):

```
Git objects:       "blob 14\0Hello, world!\n"   → глава 47
SQLite header:     "SQLite format 3\0"          → глава 21
ELF .strtab:       "\0main\0printf\0exit\0"  (таблица строк)
PE header:         "PE\0\0"
tar headers:       NUL-padded поля (имя файла до 100 байт + NUL)
```

#### NUL и определение типа файла

Наличие NUL-байтов — простейшая эвристика для различения текстовых и бинарных файлов. Утилита `file`, а также `git diff` и `grep` используют этот принцип:

```bash
# grep отказывается искать в бинарных файлах
$ grep "hello" binary.dat
Binary file binary.dat matches

# file проверяет первые байты
$ file /bin/ls
/bin/ls: Mach-O 64-bit executable x86_64

# Проверка в Python: есть ли NUL в первых 8KB?
$ python3 -c "
with open('/bin/ls', 'rb') as f:
    chunk = f.read(8192)
    print('binary' if b'\x00' in chunk else 'text')
"
binary
```
...
UTF-8 специально спроектирован так, что байт `0x00` встречается **только** как кодирование символа U+0000 (NUL) и **никогда** — внутри многобайтовых последовательностей. Это значит, что функции C-строк (`strlen`, `strchr`) корректно работают с UTF-8. Сравните с UTF-16, где NUL-байты встречаются постоянно:

```python
# UTF-8: нулевые байты только у символа NUL
>>> "Hello".encode("utf-8")
b'Hello'                         # нет 0x00

# UTF-16: нулевые байты есть у каждого ASCII-символа  
>>> "Hello".encode("utf-16-be")
b'\x00H\x00e\x00l\x00l\x00o'    # каждый второй байт — 0x00!
```

Это одна из ключевых причин победы UTF-8 в Unix-мире: **совместимость с C-строками**. Подробнее — в [главе 14](14-unicode-utf8.md#почему-utf-8-победил). А как Java обходит проблему NUL — в разделе [Modified UTF-8](14-unicode-utf8.md#modified-utf-8-mutf-8).
